---
title: August 13 EA corrected
author: G Boudinot
date: 08/13/2018
output: html_document
version: EA code version 5, C correction with N correction, GB building on Katie Snell
---
...
session<-"V12" #prefix for output .xlsx
...
colnames(rawratio)[colnames(rawratio)=="d 29N2/28N2"] <- "d15N" ####changed to 15N
...
raw.dataCarbon <- select(raw.dataCarbon, -area28, -d15N, CEnd = End, CStart = Start)


raw.dataN <- raw.data[!(is.na(raw.data$`d15N`) | raw.data$`d15N`==""), ]
raw.dataN <- subset(raw.dataN, Start >115)
raw.dataN <- subset(raw.dataN, Start <350)
raw.dataN <- select(raw.dataN, file_id, area28, d15N, NStart = Start, NEnd = End, -area44, -d13C, -Analysis, -row, -carousel, -mass, -type, -file_datetime, -Method) 


data <- full_join(raw.dataCarbon, raw.dataN, by = "file_id")
data$inv.area44<-1/data$area44
data$inv.area28<-1/data$area28

#transform(data, "Nr." = as.numeric(`Nr.`)) 

#data <- data %>% filter(`Nr.` != 3)
  
  #filter(Analysis != 14128) %>% filter(Analysis != 14132) %>% filter(Analysis != 14138) %>% filter(Analysis != 14139) %>% filter(Analysis != 14145) %>% filter(Analysis != 14164) 
...
#for (ID in c(12844:12864)){
  #ID1 <- ID-1
  #data[ data$Analysis == ID , "Identifier1"] <- data[ data$Analysis == ID1 , "Identifier1"]
 # data[ data$Analysis == ID , "mass"] <- data[ data$Analysis == ID1 , "mass"]
  #data[ data$Analysis == ID , "carousel"] <- data[ data$Analysis == ID1 , "carousel"]
  #data[ data$Analysis == ID , "type"] <- data[ data$Analysis == ID1 , "type"]
#}


#hard code
  data[ data$Analysis == 14129 , "Identifier1"] <-  "L.glut"
  data[ data$Analysis == 14129 , "mass"] <- "521"
  data[ data$Analysis == 14129 , "carousel"] <-  "4"
  data[ data$Analysis == 14129 , "type"] <-  "drift.std"
  
  data[ data$Analysis == 14130 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14130 , "mass"] <- "0"
  data[ data$Analysis == 14130, "carousel"] <-  "5"
  data[ data$Analysis == 14130 , "type"] <-  "blank"
  
  data[ data$Analysis == 14131 , "Identifier1"] <-  "OG 12-D"
  data[ data$Analysis == 14131 , "mass"] <- "40028"
  data[ data$Analysis == 14131, "carousel"] <-  "6"
  data[ data$Analysis == 14131 , "type"] <-  "sample"
  
  data[ data$Analysis == 14132 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14132 , "mass"] <- "0"
  data[ data$Analysis == 14132, "carousel"] <-  "7"
  data[ data$Analysis == 14132 , "type"] <-  "blank"
  
  data[ data$Analysis == 14133 , "Identifier1"] <-  "OG 13-D"
  data[ data$Analysis == 14133 , "mass"] <- "39990"
  data[ data$Analysis == 14133, "carousel"] <-  "8"
  data[ data$Analysis == 14133 , "type"] <-  "sample"
  
  data[ data$Analysis == 14134 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14134 , "mass"] <- "0"
  data[ data$Analysis == 14134, "carousel"] <-  "9"
  data[ data$Analysis == 14134 , "type"] <-  "blank"
  
  data[ data$Analysis == 14135 , "Identifier1"] <-  "OG 14-D"
  data[ data$Analysis == 14135 , "mass"] <- "39987"
  data[ data$Analysis == 14135, "carousel"] <-  "10"
  data[ data$Analysis == 14135 , "type"] <-  "sample"
  
  data[ data$Analysis == 14136 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14136 , "mass"] <- "0"
  data[ data$Analysis == 14136, "carousel"] <-  "11"
  data[ data$Analysis == 14136 , "type"] <-  "blank"
  
  data[ data$Analysis == 14137 , "Identifier1"] <-  "OG 15-D"
  data[ data$Analysis == 14137 , "mass"] <- "40008"
  data[ data$Analysis == 14137, "carousel"] <-  "12"
  data[ data$Analysis == 14137 , "type"] <-  "sample"
  
  data[ data$Analysis == 14138 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14138 , "mass"] <- "0"
  data[ data$Analysis == 14138, "carousel"] <-  "13"
  data[ data$Analysis == 14138 , "type"] <-  "blank"
  
  data[ data$Analysis == 14140 , "Identifier1"] <-  "LOS"
  data[ data$Analysis == 14140 , "mass"] <- "38177"
  data[ data$Analysis == 14140, "carousel"] <-  "14"
  data[ data$Analysis == 14140 , "type"] <-  "mon.std"
  
  data[ data$Analysis == 14165 , "Identifier1"] <-  "LOS"
  data[ data$Analysis == 14165 , "mass"] <- "38163"
  data[ data$Analysis == 14165, "carousel"] <-  "15"
  data[ data$Analysis == 14165 , "type"] <-  "mon.std"
  
  data[ data$Analysis == 14166 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14166 , "mass"] <- "0"
  data[ data$Analysis == 14166, "carousel"] <-  "17"
  data[ data$Analysis == 14166 , "type"] <-  "blank" 
  
  data[ data$Analysis == 14167 , "Identifier1"] <-  "OG 27-D"
  data[ data$Analysis == 14167 , "mass"] <- "39994"
  data[ data$Analysis == 14167, "carousel"] <-  "18"
  data[ data$Analysis == 14167 , "type"] <-  "sample"  

  data[ data$Analysis == 14168 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14168 , "mass"] <- "0"
  data[ data$Analysis == 14168, "carousel"] <-  "19"
  data[ data$Analysis == 14168 , "type"] <-  "blank"  
  
  data[ data$Analysis == 14169 , "Identifier1"] <-  "OG 28-D"
  data[ data$Analysis == 14169 , "mass"] <- "39998"
  data[ data$Analysis == 14169, "carousel"] <-  "20"
  data[ data$Analysis == 14169 , "type"] <-  "sample"  
  
  data[ data$Analysis == 14170 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14170 , "mass"] <- "0"
  data[ data$Analysis == 14170, "carousel"] <-  "21"
  data[ data$Analysis == 14170 , "type"] <-  "blank"  
  
  data[ data$Analysis == 14171 , "Identifier1"] <-  "OG 29-D"
  data[ data$Analysis == 14171 , "mass"] <- "40018"
  data[ data$Analysis == 14171, "carousel"] <-  "22"
  data[ data$Analysis == 14171 , "type"] <-  "sample"  
  
  data[ data$Analysis == 14172 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14172 , "mass"] <- "0"
  data[ data$Analysis == 14172, "carousel"] <-  "23"
  data[ data$Analysis == 14172 , "type"] <-  "blank"  
  
  data[ data$Analysis == 14173 , "Identifier1"] <-  "OG 30-D"
  data[ data$Analysis == 14173 , "mass"] <- "39999"
  data[ data$Analysis == 14173, "carousel"] <-  "24"
  data[ data$Analysis == 14173 , "type"] <-  "sample"  
  
  data[ data$Analysis == 14174 , "Identifier1"] <-  "Blank"
  data[ data$Analysis == 14174 , "mass"] <- "0"
  data[ data$Analysis == 14174, "carousel"] <-  "24"
  data[ data$Analysis == 14174 , "type"] <-  "blank"  
  
  data[ data$Analysis == 14175 , "Identifier1"] <-  "L.glut"
  data[ data$Analysis == 14175 , "mass"] <- "524"
  data[ data$Analysis == 14175, "carousel"] <-  "24"
  data[ data$Analysis == 14175 , "type"] <-  "drift.std"  
  
  data[ data$Analysis == 14176 , "Identifier1"] <-  "Acet. #1"
  data[ data$Analysis == 14176 , "mass"] <- "509"
  data[ data$Analysis == 14176, "carousel"] <-  "16"
  data[ data$Analysis == 14176 , "type"] <-  "dis.std"
...
```{r define-stnds}
corr.std<-"L.glut"    #you should type the name of whichever standard you used for both your lin.std and drift.std here
C.acc <- -26.39 
C.per.acc <- 40.8
N.acc <- -4.52
N.per.acc <- 9.52
...
C.acc.mon <- -27.34 
C.per.mon <- 1.66
N.acc.mon <- 6.25 #fom van Oosten's run on 20180419; corrected 0.5 per mil 
N.per.mon <- 0.133
...
stds.to.cull <- c(6, 7, 27, 28, 15, 18, 39) #6 from linearity, 27 for N, 28, 15 for C
...
assessmentC <-
  ...
  filter(Identifier1 != "Blank") %>% filter(Identifier1 != "empty")%>% filter(Identifier1 != "OG 06-D")%>% filter(Identifier1 != "OG 43-D") %>%
  # summarize information
  summarize(
    n_analyses = n(),
    d13C_mean_lindrift = mean(d13C.lindrift),
    d13C_mean_lin = mean(d13C.lin),
    d13C_mean_drift = mean(d13C.drift),
    d13C_sd = sd(d13C.lindrift),
    Ampl44_mean = mean(area44),
    Ampl44_sd = sd(area44)
  ) %>% ungroup() %>% 
...
CO2 <- assessmentC %>% 
  filter(Identifier1 != "Blank") %>% filter(Identifier1 != "empty")  %>% filter(Identifier1 != "LOS")  %>% filter(Identifier1 != "OG 19-D")  %>% filter(Identifier1 != "OG 21-D") %>% filter(Identifier1 != "L.glut") %>% filter(Identifier1 != "OG 25-D")  %>% filter(Identifier1 != "OG 23-D")  %>% filter(Identifier1 != "OG 25-D") %>%
  ggplot() + 
  aes(Identifier1, d13C_mean_lindrift) + 
  geom_errorbar(map = aes(ymin = d13C_mean_lindrift - d13C_sd, ymax = d13C_mean_lindrift + d13C_sd), show.legend = FALSE) +
 # geom_errorbarh(map = aes(xmin = Ampl28_mean - Ampl28_sd, xmax = Ampl28_mean + Ampl28_sd), show.legend = FALSE) +
  geom_point(shape = 21, size = 4) + 
  theme_bw() +
  scale_color_manual(values = c("gray", "black"))
ggplotly(CO2, dynamicTicks = TRUE)
```
...
assessmentN <-
  data %>% 
  group_by(Identifier1) %>%
  filter(Identifier1 != "blank") %>% 
  # summarize information
  summarize(
    n_analyses = n(),
    d15N_mean_lindrift = mean(d15N.lindrift),
    d15N_mean_lin = mean(d15N.lin),
    d15N_mean_drift = mean(d15N.drift),
    d15N_sd = sd(d15N.lindrift),
    Ampl28_mean = mean(area28),
    Ampl28_sd = sd(area28)
  ) %>% ungroup() %>% 
  # sort to make it easy to see
  arrange(Identifier1)
...
N2 <- assessmentN %>% 
  filter(Identifier1 != "Blank") %>%
  ggplot() + 
  aes(Identifier1, d15N_mean_lindrift) + 
  geom_errorbar(map = aes(ymin = d15N_mean_lindrift - d15N_sd, ymax = d15N_mean_lindrift + d15N_sd), show.legend = FALSE) +
 # geom_errorbarh(map = aes(xmin = Ampl28_mean - Ampl28_sd, xmax = Ampl28_mean + Ampl28_sd), show.legend = FALSE) +
  geom_point(shape = 21, size = 4) + 
  theme_bw() +
  scale_color_manual(values = c("gray", "black"))
ggplotly(N2, dynamicTicks = TRUE)
```
