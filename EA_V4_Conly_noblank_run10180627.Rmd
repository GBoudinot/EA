...
colnames(rawratio)[colnames(rawratio)=="d 29N2/28N2"] <- "d29N" ####change to 15N????
...
raw.dataCarbon <- select(raw.dataCarbon, -area28, -d29N, CEnd = End, CStart = Start)
...
raw.dataN <- select(raw.dataN, file_id, area28, d29N, NStart = Start, NEnd = End, -area44, -d13C, -Analysis, -row, -carousel, -mass, -type, -file_datetime, -Method)
...
```{r define-stnds}
corr.std<-"USGS40 L-glutamine"    #you should type the name of whichever standard you used for both your lin.std and drift.std here
C.acc <- -26.39 #this is not the final number!!!
C.per.acc <- 40.8
N.acc <- -4.52
N.per.acc <- 9.52

dis.std<-"Acetanilide #1"
C.acc.dis <- -29.53
C.per.dis <- 71.09
N.acc.dis <- 1.18
N.per.dis <- 71.09
...
mon.std<-"LOS"
C.acc.mon <- -27.34 #this is not the final number!!!
C.per.mon <- 1.66
